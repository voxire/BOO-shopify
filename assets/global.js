...
function openDrawer(drawer) {
  ...
  wireDrawerClose(drawer);
}

function wireDrawerClose(drawer) {
  if (!drawer) return;

  const overlay = drawer.querySelector('.drawer__overlay');
  const closeBtn =
    drawer.querySelector('.drawer__close') ||
    drawer.querySelector('[aria-label*="Close"]') ||
    drawer.querySelector('[data-action="close"]');
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') closeDrawer(document.querySelector('cart-drawer'));
});
...
document.addEventListener('DOMContentLoaded', () => {
  wireDrawerClose(document.querySelector('cart-drawer'));
});
